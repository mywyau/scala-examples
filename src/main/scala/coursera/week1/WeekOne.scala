...
rows  0    1                        | 1
  ...
  */
  ...
  /*  def pascal2(row: Int, column: Int): Int = {  <---- wrong
      (row, column) match {
        case (0, 0)  => 1
        case (r, c) if r == r && c == 0 => 1
        case (r, c) => pascal2(row = r, column = r - c)
      }
    }*/

  def pascal3(row: Int, column: Int): Int = { // <--- correct using math!!!!
    (row, column) match {
      case (_, 0) => 1
      case (r, c) =>
  ...
  // need to create two loops probs one to calc the fst number above the other to calc the snd number above
  //  def pascalT(column: Int, row: Int): Int = {
  //
  //    @tailrec
  //    def lastRowNumber(c: Int, r: Int, acc:Int): Int = {
  //      (c, r) match {
  //        case (c, r) if c == 0 | r - 1 == c => 1
  //        case (c, r) =>
  //          lastRowNumber(c = c , r = r - 1,  values = (,), total = )
  //      }
  //    }
  //
  //    lastRowNumber(column, row, 0)
  //  }
