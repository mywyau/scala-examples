...
object CatsIntro extends App {
  ...
  case class ToyCar(
      model: String,
      price: Double
  ) // note never actually use Double type for money
  ...
  val compareTwoToyCars =
    ToyCar("Ferarri", 29.99) === ToyCar("Lamborghini", 29.99) // true
  val neqTwoToyCars =
    ToyCar("Ferarri", 30.00) === ToyCar(
      "Lamborghini",
      29.99
    ) // false because of our rule/law for equality

  println(
    s"neqTwoToyCars: are these cars of price $$30.00 and $$£29.99 equal? $neqTwoToyCars"
  ) // haha forgot $ dollar signs need double $$
